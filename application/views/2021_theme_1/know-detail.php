			...
			<div class="col-lg-7 col-sm-6 ">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item color-menu"><a href="<?=base_url()?>#fourthPage"><?=lang('Knowledge') ?></a></li>
						<?/*?>
						<li class="breadcrumb-item  color-menu-active active" aria-current="page"><?=$blog->blog_name?></li>
						<?*/?>
					</ol>
				</nav>
			</div>
	...
	<div class="container">
		<div class="row">
			<div class="col-lg-8 " style="animation-duration: 1.5s;">
				<h4 class="name-knowledge"><?=$blog->blog_name?></h4>
				<ul class="ul-knowledge ">
					<li>
						<?=lang('By') ?> : <span style="color: #808080;"><?=$blog->cby?></span>
					</li>
					<li>
						<?=lang('Posted date') ?> : <span style="color: #808080;"><?=date("d-m-Y", strtotime($blog->cdate))?></span>
					</li>
					<li>
						<?=lang('Number Of Visitors') ?> : <span style="color: #808080;"><?=$blog->count?></span>
					</li>
				</ul>
				...
				<ul class="ul-share ul-knowledge-share">
					...
					<?php 	$userinput = base_url($this->session->userdata('site_lang_name')."/Blog/detail/$blog->blog_id"); 
					...
					<li>
						<div class="share-mail">
							<a href='mailto:?body=<?=base_url()?><?=$this->session->userdata('site_lang_name')?>/Blog/detail/<?=$blog->blog_id?>'>
				...
				<h5 class="h3-know-advice" style="font-weight: bold; ">
					<?=lang('Related Blog') ?> </h5>
								...
								<? foreach ($blogall as $blogalls) { ?>
									<div class="item">
										<div class="knowledge-about">
											<a href="<?=base_url()?><?=$this->session->userdata('site_lang_name')?>/Blog/detail/<?=$blogalls->blog_id?>">
												<? foreach ($blogalls->images as $key => $images) { ?>
													<? if ($key==0) { ?>
														...
														<picture>
															<?php foreach ($i as $key => $newi) { ?>
																...
																<?php } ?>
																<img class="lazy img-fluid img-thumbnail img-thumbnail-no-borders rounded-0" data-src="<?=base_url()?>/images/website/<?=$images->filename ?>_<?=$images->show_admin?>.<?=$images->extension ?>" alt="<?=$blogalls->blog_name?>">
															</picture>
														<? } ?>
													<? } ?>
												</a>
									...
									<div class="row border-tab">
										<? foreach ($Allpop as $Allpops) { ?>
											<div class="col-lg-12 margin-top">
												<div class="row">
													<div class="col-md-4">
														<a href="<?=base_url()?><?=$this->session->userdata('site_lang_name')?>/Blog/detail/<?=$Allpops->blog_id?>">
																...
																<? if ($key==0) { ?>
																	...
																	<picture>
																		<?php foreach ($i as $key => $newi) { ?>
																			<source media="(min-width:<?=$key;?>px)" srcset="<?=base_url()?>/images/website/<?=$images->filename ?>_<?=$newi?>.<?=$images->extension ?>">
																			<?php } ?>
																			<img class="lazy img-fluid img-thumbnail img-thumbnail-no-borders rounded-0" data-src="<?=base_url()?>/images/website/<?=$images->filename ?>_<?=$images->show_admin?>.<?=$images->extension ?>" alt="<?=$Allpops->blog_name?>">
																		</picture>
																	<? } ?>
																<? } ?>
															</a>
